...
def main():
    NUMBEROFCONNECTIONTOBEMADE = int(input("Number Of Connections to made: "))

    host = Host()

    # set number of threads to 10000
    ...
    for i in range(NUMBEROFCONNECTIONTOBEMADE):
        threading.Thread(
            target=client_program,
            args=(socket.gethostbyaddr("127.0.0.1")[0], 8888),
        ).start()
        print("Connection made " + str(i))
